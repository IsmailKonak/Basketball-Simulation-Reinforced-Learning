...
# Importing the model trained
file = open("trained_model01.pkl","rb+")
prob = pickle.load(file)
degrees = []
for i in range(1,90):
    degrees.append(i)

#Set the probabilities of each angle from the trained model
prob = np.array(prob)
...
while True:
    for i in range(10):
        ...
        cur = np.random.choice(degrees,p = prob)-1
        print("Chosen angle: "+str(cur))
        sim.main(cur)
        sim.info()
